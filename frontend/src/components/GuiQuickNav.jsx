...
export default function GuiQuickNav({ stateAbbr }) {
  ...
  const linkClassName = ({ isActive }) => (
    `gui-quick-nav__link${isActive ? ' gui-quick-nav__link--active' : ''}`
  );
  ...
  return (
    <nav className="gui-quick-nav" aria-label="GUI use case routes">
      <div className="gui-quick-nav__header">
        ...
        <NavLink
          className={({ isActive }) => `gui-quick-nav__link gui-quick-nav__link--home${isActive ? ' gui-quick-nav__link--active' : ''}`}
          to={`/state/${stateAbbr}`}
          end
        >
          Overview
      ...
      <div className="gui-quick-nav__grid">
        {orderedSlugs.map((slug) => (
          <NavLink
            ...
            to={slug === 'gui-24' ? '/' : `/state/${stateAbbr}/gui/${slug}`}
          >
